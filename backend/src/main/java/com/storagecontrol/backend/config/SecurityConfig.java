...
import com.storagecontrol.backend.auth.filter.JwtAuthenticatorFilter;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)
            ...
            .authorizeHttpRequests(auth -> auth.requestMatchers(
                ...
                "/api/v1/auth/login",
                "/actuator/health"
            ).permitAll().anyRequest().authenticated()).addFilterBefore(jwtAuthenticatorFilter, UsernamePasswordAuthenticationFilter.class);
